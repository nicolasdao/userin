...
const OAUTH_PATHNAME = '/google/oauth2'
const OAUTH_CALLBACK_PATHNAME = '/google/oauth2callback'
...
const getAuthRequestHandler = ({ appId, appSecret, scopes }) => (req, res, next) => {
	...
	const handler = passport.authenticate('google', { scope: scopes })
...
const getAuthResponseHandler = ({ userPortalAPI, apiKey, onSuccess, onError }) => (req, res, next) => {
	...
	const handler = passport.authenticate('google', {}, (err,user) => {
		// CASE 1 - IdP Failure
		if (err) {
